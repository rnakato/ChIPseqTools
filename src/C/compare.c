...
/*  if (peakset[sample2].bsarray[j].enrich) {
         peakset[sample1].bsarray_overlap[sample2][num].ratio_enrich = peakset[sample1].bsarray[i].enrich / peakset[sample2].bsarray[j].enrich;
  } else peakset[sample1].bsarray_overlap[sample2][num].ratio_enrich = 0;*/

  if (peakset[sample2].bsarray[j].maxIP) {
        peakset[sample1].bsarray_overlap[sample2][num].ratio_maxIP = peakset[sample1].bsarray[i].maxIP / peakset[sample2].bsarray[j].maxIP;
...
static void getoverlap(struct peakset *peakset, int sample1, int sample2, int i, int j, int num)
{
  ...
  int base_overlap = min(peakset[sample1].bsarray[i].end,   peakset[sample2].bsarray[j].end)
                   - max(peakset[sample1].bsarray[i].start, peakset[sample2].bsarray[j].start);
  if (base_overlap < 0) base_overlap = 0;
  peakset[sample1].base_overlap[sample2] += base_overlap;
}
